...
import static io.cloudslang.content.blueocean.entities.constants.Inputs.TokenInputs.EXPIRY_TIME_IN_MINS;
...
public class ChangeTokenExpiryTime {
    /**
     * Changes token default expiring time.
     * JWT tokens expires after 30 minutes by default. Expire claim header gives the time at which token expires. It is
     * unix time in seconds. Default 30 minutes can be changed by sending expiryTimeInMins query parameter. This parameter
     * must be used carefully, it has security implications.
     * https://github.com/jenkinsci/blueocean-plugin/blob/master/blueocean-jwt/README.md#change-expiry-time
     *
     ...
     *                             Valid values: "true", "false"
     *                             Default value: "true"
     * @param expiryTimeInMins     Optional - specifies after how many minutes JWT tokens expires.
     *                             Valid values: positive value less than maximum expiry time allowed -> 480 minutes
     *                             Default value: "30"
     * @return A map with strings as keys and strings as values that contains: outcome of the action (or failure message
     * and the exception if there is one), returnCode of the operation and the ID of the request
     */
    @Action(name = "ChangeTokenExpiryTime",
            outputs = {
                    @Output(RETURN_CODE),
    ...
    public Map<String, String> changeTokenExpiryTime(@Param(value = PROTOCOL) String protocol,
                                                     ...
                                                     @Param(value = EXPIRY_TIME_IN_MINS) String expiryTimeInMins) {
        try {
            ...
            final TokenInputs tokenInputs = new TokenInputs.Builder()
                    .withExpiryTimeInMins(expiryTimeInMins)
                    .build();
            ...
            return new BlueOceanService().execute(wrapper);
